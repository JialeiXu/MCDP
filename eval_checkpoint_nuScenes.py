import numpy as np
import torch.nn
from dataload_nuScenes import nuScenes_RAWDataset
...
class val_Options:
    def __init__(self):
        ...
        self.parser.add_argument('--json_file_val', type=str,
                                 default='./datasets/new_mask_data_Cam_01.json')
        self.parser.add_argument('--self_mask', type=list,
                                 default=['./mask/1_mask_black.png',
                                          './mask/5_mask_black.png',
                                          './mask/6_mask_black3.png',
                                          './mask/7_mask_black.png',
                                          './mask/8_mask_black2.png',
                                          './mask/9_mask_black2.png'])
        ...
        self.parser.add_argument("--height",type=int,help="input image height",default=448)
        self.parser.add_argument("--width",type=int,help="input image width",default=768)
...
if __name__=='__main__':
    ...
    options = val_Options()
    args = options.parse()
    json_file_list = ['./datasets/nuScenes/sweep/sweeps_F.json',
                      './datasets/nuScenes/sweep/sweeps_FL.json',
                      './datasets/nuScenes/sweep/sweeps_FR.json',
                      './datasets/nuScenes/sweep/sweeps_BL.json',
                      './datasets/nuScenes/sweep/sweeps_BR.json',
                      './datasets/nuScenes/sweep/sweeps_B.json',
                      ]
    if args.depth_code and args.code_num>1:
    ...
    real_refine_times = args.refine_times
    ...
    for json_file_i in json_file_list:
        ...
        val_loader = DataLoader(
            val_dataset, batch_size=args.batch_size, num_workers=10,drop_last=True,shuffle=True)
        models = {}
        ...
        if args.code_num > 1 and args.depth_code:
            if args.GRU:
                ...
                for refine_time_i in range(args.refine_times):
                    models['refine_net_'+str(refine_time_i)] = networks.wight_net(args,models["encoder"].num_ch_enc,B=args.batch_size).cuda()
